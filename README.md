...
| **智能缓存** | 基于文件哈希的缓存，避免重复处理（PDF+音频） |
| **分块总结** | 处理超长文本，突破模型上下文窗口限制 |
| **费用跟踪** | 自动跟踪 API 使用量和费用（可选配置） |
| **AI 标题生成** | 自动从笔记内容提取标题，生成有意义的文件名 |
...
**A:** 默认存储在 `./cache/` 目录，包含：
- `ocr/` - OCR 结果缓存（按 PDF 文件哈希组织）
- `stt/` - STT 结果缓存（按音频文件哈希组织）
- `knowledge_bases.json` - 知识库配置
- `kb_indices/` - 知识库索引

缓存基于文件内容的 MD5 哈希验证，文件修改后会自动重新处理。
...
**新增：页面级缓存与断点续传**
- 新增页面级缓存功能，每页 OCR 结果独立缓存
- 处理中断后可从上次未完成的页面继续处理
- 支持两种缓存格式：OCRResult 格式 (`.ocr.json`) 和 API 转储格式 (`.json`)
- PDF 图片导出优化：自动检测并复用已存在的图片，避免重复转换
- GUI 模式现在与 CLI 模式拥有相同的缓存和恢复能力
...
**测试：**
- 整体测试覆盖率 60%（176 个测试全部通过）

### [0.8.0] - 2024-12-30

**新增：STT 缓存支持**
- 新增 `STTCache` 类，为音频处理提供基于文件哈希的缓存功能
- 新增 `AudioCache` 数据模型，存储音频转录结果
- `CacheLoader` 新增 STT 缓存加载方法，支持将 STT 缓存作为知识库信源
- 音频处理管线现在支持缓存，避免重复转录
- STT 请求超时时间从 10 分钟调整为 30 分钟，支持处理大文件

**新增：AI 标题生成**
- `SummarizationWorker` 新增 `generate_title()` 方法
- 自动从笔记内容中提取各级标题，使用 AI 生成有意义的文件名
- 支持标题清理和文件名安全化处理
- PDF 和音频处理生成的笔记文件名不再使用时间戳，而是 AI 生成的描述性标题

**测试：**
- 新增 STTCache 单元测试（13 个新测试）
- 新增 AudioCache 模型测试（3 个新测试）
- 新增 generate_title 方法测试（3 个新测试）
- 整体测试覆盖率 63%（176 个测试全部通过）

### [0.7.3] - 2024-12-30

**新增：自动重试机制**
- BaseWorker 新增智能重试功能，自动重试失败的 API 请求
- 支持可配置的最大重试次数和重试延迟（通过环境变量）
- 智能异常分类：自动识别可重试错误（超时、网络错误、429/503/502）和不可重试错误（ValueError、TypeError 等）
- 重试统计跟踪：记录总重试次数和失败次数
- Worker 关闭时显示重试统计信息

**更新：环境变量配置**
- .env.example 新增"Retry Configuration"章节
- 支持为每个服务独立配置重试参数（OCR/ASR/SUMMARIZATION/ASK_AI）
- 默认配置：OCR 2 次重试，其他服务 0 次重试

**改进：API 使用情况处理**
- 当 API 不返回 token 使用情况时，记录 warning 而不是崩溃
- 费用跟踪在缺少使用信息时自动设置为 0

**测试：**
- 新增重试机制单元测试（6 个新测试）
- 新增 retrieval 模块测试（29 个新测试）
- 整体测试覆盖率从 43% 提升到 66%（158 个测试）
